//APISearchResultsWithKeenSlider.tsx
...
const SimpleList = ({ data }: Props) => {
  ...
  const listOfAllURLS = searchResultsdata?.items.map((x) => (
    x.has_image ? x.image_thumbnail : null
  ));
  console.log(listOfAllURLS);
    ...
    <ListGroup.Item>
      ...
      {!x.has_image ? (
        <div>
          ...
          <Image src={defaultImage} alt={"showing default image"} fluid></Image>
        </div>
      ) : (
        <div>
          {/* <p>Image Available</p>
...
const APISearchResultsWithKeenSlider = () => {
...
export default APISearchResultsWithKeenSlider;
